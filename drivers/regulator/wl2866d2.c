...
#define wl2866d2_vdd_step(type) (type ? 12500 : 6000)
...
static int wl2866d2_masked_write(struct regmap *regmap, u8 reg, u8 mask, u8 val)
{
	int rc;
...
static int wl2866d2_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	rc = wl2866d2_read(wl2866d2_reg->regmap, WL2866D2_ENABLE_REG, &en_value,
			   1);
	if (rc < 0) {
		wl2866d2_err(wl2866d2_reg,
	...
	wl2866d2_debug(wl2866d2_reg, "addr:0x%x  0x%x  is enabled = %d\n",
		       WL2866D2_ENABLE_REG, en_value,
		       !!(en_value & 1 << wl2866d2_reg->index));
				  ...
				  int min_uv, int max_uv)
{
	...
	rc = wl2866d2_write(wl2866d2_reg->regmap, wl2866d2_reg->addr,
			    &reg_voltage, 1);
	if (rc < 0) {
				      ...
				      struct device_attribute *attr, char *buf)
{
	return sprintf(
		buf,
		"===================== PMU wl2866d2 LDO debug node operation instructions =====================\n"
...
static DEVICE_ATTR(wl2866d2_OpsInstructions, 0644, wl2866d2_DebugOps_show,
		   NULL);
...
static DEVICE_ATTR(wl2866d2_ShowAllLdoState, 0644, wl2866d2_allldostate_show,
		   NULL);
...
static DEVICE_ATTR(wl2866d2_LDOSetGetVol, 0644, wl2866d2_LDOSetGetVol_show,
		   wl2866d2_LDOSetGetVol_store);
...
static DEVICE_ATTR(wl2866d2_ReadWriteReg, 0644, wl2866d2reg_show,
		   wl2866d2reg_store);
...
static DEVICE_ATTR(wl2866d2_LDOEnDisable, 0644, wl2866d2_LDOEnDisable_show,
		   wl2866d2_LDOEnDisable_store);
				 ...
				 const char *name)
{
	...
	init_data =
		of_get_regulator_init_data(dev, of_node, &wl2866d2_reg->rdesc);
	if (init_data == NULL) {
	...
	wl2866d2_reg->rdesc.n_voltages =
		((reg_data[i].max_uv - reg_data[i].min_uv) /
		 wl2866d2_reg->rdesc.uV_step) +
		1;

	wl2866d2_reg->rdev =
		devm_regulator_register(dev, &wl2866d2_reg->rdesc, &reg_config);
			       ...
			       const struct i2c_device_id *id)
{
	...
	chip->dev = &client->dev;
	chip->regmap =
		devm_regmap_init_i2c(client, &wl2866d2_regulator_regmap_config);
	if (!chip->regmap)
		return -ENODEV;
	...
	for_each_available_child_of_node (chip->dev->of_node, child) {
		wl2866d2_reg = devm_kzalloc(chip->dev, sizeof(*wl2866d2_reg),
					    GFP_KERNEL);
		if (!wl2866d2_reg)
			return -ENOMEM;
	...
	rc = sysfs_create_group(&client->dev.kobj,
				&wl2866d2_debug_attribute_group);
	if (rc)
