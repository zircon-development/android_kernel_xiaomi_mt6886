...
static unsigned int ln8410_reg_list_b[] = {
	...
	0x2D, 0x2E, 0x30, 0x31, 0x52, 0x54, 0x59, 0x60, 0x61, 0x62, 0x63, 0x69,
	0x76, 0x79, 0x7B, 0x7E, 0x80, 0x8D, 0x98, 0x99, 0x9A, 0x9B, 0x9C,
};
...
static int ln8410_set_regn_pull_down(struct ln8410_device *chip, bool enable)
{
	...
	if (enable) {
		ret = regmap_update_bits(chip->regmap,
					 LN8410_REG_TEST_MODE_CTRL_2, 0x80,
					 0x80); /* FORCE_VWPC_PD = 1 */
		ret = regmap_update_bits(chip->regmap,
					 LN8410_REG_TEST_MODE_CTRL_2, 0x40,
					 0x00); /* TM_VWPC_PD = 0 */
		ret = regmap_update_bits(chip->regmap, LN8410_REG_FORCE_SC_MISC,
					 0x04, 0x04); /* TM_REGN_PD = 1 */
	} else {
		ret = regmap_update_bits(chip->regmap,
					 LN8410_REG_TEST_MODE_CTRL_2, 0x80,
					 0x00); /* FORCE_VWPC_PD = 0 */
		ret = regmap_update_bits(chip->regmap,
					 LN8410_REG_TEST_MODE_CTRL_2, 0x40,
					 0x00); /* TM_VWPC_PD = 0 */
		ret = regmap_update_bits(chip->regmap, LN8410_REG_FORCE_SC_MISC,
					 0x04, 0x00); /* TM_REGN_PD = 0 */
	}
...
static int ln8410_set_adc_mode(struct ln8410_device *chip, u8 mode)
{
	...
	if (mode == 0x00) {
		...
		ret = regmap_write(chip->regmap, LN8410_REG_ADC_FN_DISABLE1,
				   0xFF);
		ret = regmap_update_bits(chip->regmap, LN8410_REG_ADC_CTRL2,
					 0xC0, 0xC0); /* normal mode */
		ret = regmap_update_bits(chip->regmap, LN8410_REG_LION_CFG_1,
					 0x11,
					 0x10); /* ADC_SKIP_IDLE=1, OSR=64 */
...
static int ln8410_enable_qb(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_0A, LN8410_QB_EN_MASK,
				 val);
...
static int ln8410_get_charge_enable(struct ln8410_device *chip, bool *enabled)
{
	...
	*enabled =
		!(val & (LN8410_SHUTDOWN_STS_MASK | LN8410_STANDBY_STS_MASK));
...
static int ln8410_set_div_mode(struct ln8410_device *chip, int mode)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_0D, LN8410_MODE_MASK,
				 val);
...
static int ln8410_enable_batovp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_01,
...
static int ln8410_enable_batocp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_02,
...
static int ln8410_set_usbovp(struct ln8410_device *chip, int threshold)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_03,
...
static int ln8410_set_wpcovp(struct ln8410_device *chip, int threshold)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_04,
...
static int ln8410_set_busovp(struct ln8410_device *chip, bool threshold)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_0D,
...
static int ln8410_set_outovp(struct ln8410_device *chip, int threshold)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_79,
...
static int ln8410_enable_busocp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_05,
...
static int ln8410_enable_busucp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_06,
...
static int ln8410_set_busucp(struct ln8410_device *chip, int threshold)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_7C,
...
static int ln8410_enable_pmid2outovp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_07,
...
static int ln8410_enable_pmid2outuvp(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_08,
...
static int ln8410_enable_adc(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_14,
...
static int ln8410_get_adc(struct ln8410_device *chip, int channel, int *result)
{
	...
	regmap_update_bits(chip->regmap, LN8410_REG_76,
			   LN8410_PAUSE_ADC_UPDATES_MASK,
			   LN8410_PAUSE_ADC_UPDATES_MASK);
	msleep(2); // 2 MS

	ret = regmap_read(chip->regmap, LN8410_REG_16 + (channel << 1), &val_h);
	ret = regmap_read(chip->regmap, LN8410_REG_16 + (channel << 1) + 1,
			  &val_l);

	/*PAUSE_ADC_UPDATES=0 */
	regmap_update_bits(chip->regmap, LN8410_REG_76,
			   LN8410_PAUSE_ADC_UPDATES_MASK, 0);
...
static int ln8410_set_ss_timeout(struct ln8410_device *chip, int timeout)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_0C,
				 LN8410_SS_TIMEOUT_SET_MASK, val);
...
static int ln8410_vbus_short_dis(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_7B,
				 LN8410_VBUS_SHORT_DIS_MASK, val);
	ret = regmap_update_bits(chip->regmap, LN8410_REG_OVPGATE_CTRL_0,
				 LN8410_OVPGATE1_PD_CFG_MASK,
				 0x3 << LN8410_OVPGATE1_PD_CFG_SHIFT);
	ret = regmap_update_bits(chip->regmap, LN8410_REG_OVPGATE_CTRL_0,
				 LN8410_OVPGATE2_PD_CFG_MASK,
				 0x3 << LN8410_OVPGATE2_PD_CFG_SHIFT);
...
static int ln8410_use_hvldo(struct ln8410_device *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_52,
...
static int ln8410_fsw_set(struct ln8410_device *chip, u8 date)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_0B,
...
static int ln8410_soft_reset(struct ln8410_device *chip)
{
	...
	ret = regmap_update_bits(chip->regmap, LN8410_REG_TEST_MODE_CTRL, 0x1,
				 0x1);
	msleep(30);
	...
	if (i == 30)
		xmc_info(
			"%s fail to reset, can't check the valid status(sys_st=0x%x)\n",
			chip->log_tag, sys_st);
...
static int protected_reg_init(struct ln8410_device *chip)
{
	...
	if (chip->revision == 0x2) {
		...
		ret = regmap_update_bits(chip->regmap,
					 LN8410_REG_LION_STARTUP_CTRL, 0x04,
					 0x04); /* C1_C2_CHARGE_TIME_CFG = 1 */
		ret = regmap_update_bits(chip->regmap, LN8410_REG_CFG9, 0x20,
					 0x20); /* USE_TINY_MASK = 1 */
		ret = regmap_update_bits(
			chip->regmap, LN8410_REG_CFG8, 0x40,
			0x00); /* C1N_C2N_C3N_FORCE_IDLE_MASK = 0 */
	}
...
static int ln8410_init_protection(struct ln8410_device *chip, int mode)
{
	...
	ret = ln8410_set_busovp(
		chip, 0); /*0 low set, 5.6v for 1:1, 12v for 2:1, 22v for 4:1*/
...
static int ln8410_init_device(struct ln8410_device *chip, int driver_data)
{
	...
	ret = regmap_read(chip->regmap, LN8410_REG_PRODUCT_ID,
			  &(chip->product_cfg));
	chip->product_cfg = chip->product_cfg & 0x0F;

	ln8410_soft_reset(chip);
	regmap_write(chip->regmap, LN8410_REG_RECOVERY_CTRL,
		     0x00); // turn off all auto recovery
	...
	regmap_update_bits(chip->regmap, LN8410_REG_11, 0xFE,
			   0xFE); //VUSB INSERT
	regmap_update_bits(chip->regmap, LN8410_REG_14, 0x08,
			   0x08); //close adc done irq
	regmap_update_bits(chip->regmap, LN8410_REG_07, 0x10,
			   0x10); //close pmic2out ovp irq
	regmap_update_bits(chip->regmap, LN8410_REG_08, 0x10,
			   0x10); //close pmic2out uvp irq
	regmap_update_bits(chip->regmap, LN8410_REG_LION_INT_MASK, 0xFF, 0xFF);
	regmap_update_bits(chip->regmap, LN8410_REG_06, 0x0A,
			   0x0A); //close ibus ucp rise and fail irq
	regmap_update_bits(chip->regmap, LN8410_REG_04, 0x40,
			   0x40); //close vwpc ovp irq
	regmap_update_bits(chip->regmap, LN8410_REG_03, 0x40,
			   0x40); //close vusb ovp irq
	regmap_update_bits(chip->regmap, LN8410_REG_01, 0x20,
			   0x20); //close vbat ovp irq
...
static void ln8410_dump_register(struct ln8410_device *chip)
{
	...
	int i = 0, reg_num_a = ARRAY_SIZE(ln8410_reg_list_a),
	    reg_num_b = ARRAY_SIZE(ln8410_reg_list_b);
...
static int ln8410_ops_get_charge_enable(struct charger_device *chg_dev,
					bool *enabled)
{
	struct ln8410_device *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int ln8410_ops_set_div_mode(struct charger_device *chg_dev, int mode)
{
	...
	switch (mode) {
	case XMC_CP_1T1:
		ret = ln8410_set_div_mode(chip,
					  LN8410_FORWARD_1_1_CHARGER_MODE);
		break;
	case XMC_CP_2T1:
		ret = ln8410_set_div_mode(chip,
					  LN8410_FORWARD_2_1_CHARGER_MODE);
		break;
	case XMC_CP_4T1:
		ret = ln8410_set_div_mode(chip,
					  LN8410_FORWARD_4_1_CHARGER_MODE);
		break;
	default:
...
static int ln8410_ops_device_init(struct charger_device *chg_dev, int mode)
{
	...
	switch (mode) {
	case XMC_CP_1T1:
		ret = ln8410_init_protection(chip,
					     LN8410_FORWARD_1_1_CHARGER_MODE);
		break;
	case XMC_CP_2T1:
		ret = ln8410_init_protection(chip,
...
static int ln8410_ops_get_bypass_support(struct charger_device *chg_dev,
					 bool *enabled)
{
	*enabled = 1;
	xmc_err("%s %d\n", __func__, *enabled);
...
static int ln8410_ops_get_chip_vendor(struct charger_device *chg_dev,
				      int *chip_id)
{
	struct ln8410_device *chip = charger_get_data(chg_dev);
...
static int ln8410_ops_enable_acdrv_manual(struct charger_device *chg_dev,
					  bool enable)
{
	struct ln8410_device *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int ln8410_register_charger(struct ln8410_device *chip, int driver_data)
{
	...
	case LN8410_SLAVE:
		chip->chg_dev =
			charger_device_register("cp_slave", chip->dev, chip,
						&ln8410_ops,
						&ln8410_slave_chg_props);
		break;
	case LN8410_MASTER:
		chip->chg_dev =
			charger_device_register("cp_master", chip->dev, chip,
						&ln8410_ops,
						&ln8410_master_chg_props);
		break;
	default:
		return -EINVAL;
		break;
...
static void ln8410_irq_handler(struct work_struct *work)
{
	struct ln8410_device *chip =
		container_of(work, struct ln8410_device, irq_handle_work.work);
	ln8410_dump_register(chip);
	xmc_info("%s", __func__);
...
static int ln8410_init_irq(struct ln8410_device *chip)
{
	...
	ret = request_irq(chip->irq, ln8410_interrupt,
			      ...
			      const char *buf, size_t count)
{
	...
	usb_attr = container_of(attr, struct mtk_cp_sysfs_field_info, attr);
...
static struct attribute *cp_sysfs_attrs[ARRAY_SIZE(cp_sysfs_field_tbl) + 1];
...
static int cp_sysfs_create_group(struct power_supply *psy)
{
	...
	return sysfs_create_group(&psy->dev.kobj, &cp_sysfs_attr_group);
}
...
static int ln8410_get_property(struct power_supply *psy,
...
static int ln8410_psy_init(struct ln8410_device *chip, struct device *dev,
			   int driver_data)
{
	...
	chip->cp_psy = devm_power_supply_register(chip->dev, &chip->psy_desc,
						  &psy_cfg);
...
static int ln8410_detect_device(struct i2c_client *client,
				struct ln8410_device *chip, int driver_data)
{
	int retry_count = 0;
	s32 data = 0;
...
static int ln8410_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	chip->irq_wakelock =
		wakeup_source_register(NULL, "ln8410_irq_wakelock");
