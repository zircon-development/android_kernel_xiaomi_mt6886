			       ...
			       void *data)
{
	...
	debugfs_create_file("status", S_IFREG | 0444, votable->root, votable,
			    &votable_status_ops);

	debugfs_create_u32("force_val", S_IFREG | 0644, votable->root,
			   &(votable->force_val));

	debugfs_create_file("force_active", S_IFREG | 0444, votable->root,
			    votable, &votable_force_ops);

	debugfs_create_u32("effective_val", S_IFREG | 0644, votable->root,
			   &(votable->effective_result));

	debugfs_create_file("effective_client", S_IFREG | 0444, votable->root,
			    votable, &effective_client_ops);
	return votable;
}
