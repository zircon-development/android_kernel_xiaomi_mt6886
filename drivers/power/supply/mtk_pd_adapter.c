		...
		int mV, int mA)
{
	...
	if (info->adapter_dev->verify_process) {
		pr_notice("verify_processing, skip pd_set_cap_xm\n");
		return -1;
	}
...
static int pd_set_pd_verify_process(struct adapter_device *dev, int verifying)
{
	...
	pr_notice("[%s] pd verify in process:%d\n", __func__, verifying);
...
static int pd_get_svid(struct adapter_device *dev)
{
	...
	if (info->adapter_svid_list == NULL) {
		...
		if (info->adapter_svid_list == NULL)
			pr_notice("[%s] adapter_svid_list is still NULL!\n", __func__);
	}
	...
	if (ret == TCPM_SUCCESS) {
		...
		if (ret == TCPM_SUCCESS) {
			...
			pr_notice("[%s] get adapter message failed!\n", __func__);
			return MTK_ADAPTER_ERROR;
		}
	}
	...
	unsigned int data_len)
{
	...
	case USBPD_UVDM_REVERSE_AUTHEN:
		...
		break;
	default:
		pr_notice("cmd:%d is not support\n", cmd);
		break;
	}
...
static int pd_get_pdos(struct adapter_device *dev)
{
	...
	pr_notice("[%s] tcpm_inquire_pd_source_cap is %d.\n", __func__, ret);
	if (ret)
		return MTK_ADAPTER_ERROR;
	for (i = 0; i < 7; i++) {
		info->adapter_dev->received_pdos[i] = cap.pdos[i];
		pr_notice("[%s]: pdo[%d] { received_pdos is %08x, cap.pdos is %08x}\n",
	...
	struct adapter_power_cap *tacap)
{
	...
	if (info->adapter_dev->verify_process) {
		pr_notice("verify_processing, skip pd_get_cap\n");
		return -1;
	}
...
APDO_REGAIN:
		...
		if (pd_cap.nr != 0) {
			...
			for (i = 0; i < pd_cap.nr; i++) {
				...
				pr_notice("[%s]VBUS = [%d,%d], IBUS = %d, WATT = %d, TYPE = %d\n",
		...
		while (timeout < 15) {
			...
			pr_notice("[%s] ret=%d\n", __func__, ret);
			if (ret == TCPM_SUCCESS) {
				pr_notice("[%s] ready to get pps info\n", __func__);
				ret = usb_set_property(USB_PROP_PD_AUTHENTICATION, 1);
				if (ret < 0)
					pr_notice("[%s] failed to set authentication\n", __func__);
				goto APDO_REGAIN;
			} else {
				pr_notice("[%s] retry times = %d, for PPS ready\n", __func__, timeout);
				timeout++;
				msleep(80);
			}
		}
	} else if (type == MTK_CAP_TYPE_UNKNOWN) {
		pr_notice("[%s] xiaomi pd adapter auth failed\n", __func__);
