...
static struct bq_fg_batt_adapting_power battery_adapting_power[] = {
	{ .letter = '0', .power = 10 },	 { .letter = '1', .power = 15 },
	{ .letter = '2', .power = 18 },	 { .letter = '3', .power = 25 },
	{ .letter = '4', .power = 33 },	 { .letter = '5', .power = 35 },
	{ .letter = '6', .power = 40 },	 { .letter = '7', .power = 55 },
	{ .letter = '8', .power = 60 },	 { .letter = '9', .power = 67 },
	{ .letter = 'A', .power = 80 },	 { .letter = 'B', .power = 90 },
	{ .letter = 'C', .power = 100 }, { .letter = 'D', .power = 120 },
	{ .letter = 'E', .power = 140 }, { .letter = 'F', .power = 160 },
	{ .letter = 'G', .power = 180 }, { .letter = 'H', .power = 200 },
	{ .letter = 'I', .power = 220 }, { .letter = 'J', .power = 240 },
};
...
retry:
	...
	if (ibat == 0 &&
	    bq->bq_charging_status == POWER_SUPPLY_STATUS_DISCHARGING)
		ibat = -1 * bq->ibat;
					 ...
					 s32 system_soc_now)
{
	...
	if (!is_charger_exist || system_soc_now <= 92 || temp > 470 ||
	    temp < 150 || rsoc < 75 || bq->smart_chg_navigation_en) {
		if (!is_charger_exist) {
...
static enum power_supply_property fg_props[] = {
	...
	POWER_SUPPLY_PROP_MODEL_NAME,  POWER_SUPPLY_PROP_TECHNOLOGY,
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (!strncmp(data, "MI", 2)) {
		...
		if (bq->chip_ok) {
			for (i = 0; i < ARRAY_SIZE(battery_adapting_power);
			     i++) {
				if (data[4] ==
				    battery_adapting_power[i].letter) {
					bq->adapting_power =
						battery_adapting_power[i].power;
					break;
				}
			}
		}
