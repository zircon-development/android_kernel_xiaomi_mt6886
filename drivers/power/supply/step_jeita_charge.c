...
static void get_index(struct step_jeita_cfg0 *cfg, int fallback_hyst,
		      int forward_hyst, int value, int *index, bool ignore_hyst,
		      int tuple_count)
{
	int new_index = 0, i = 0;
	...
	for (i = 0; i < tuple_count; i++) {
		if (is_between(cfg[i].low_threshold, cfg[i].high_threshold,
			       value)) {
			new_index = i;
			break;
		}
	...
	if (ignore_hyst) {
		...
		if (new_index > index[0]) {
			if (value <
			    (cfg[new_index].low_threshold + forward_hyst))
				new_index = index[0];
		} else if (new_index < index[0]) {
			if (value >
			    (cfg[new_index].high_threshold - fallback_hyst))
				new_index = index[0];
		}
		index[1] = index[0];
		index[0] = new_index;
...
static int handle_battery_suppliers(struct mtk_charger *info)
{
	...
	case BATTERY_SUPPLIER_SUNWODA:
		total_length = of_property_count_elems_of_size(
			np, "jeita_iterm_cfg_second_supplier", sizeof(u32));
		if (total_length < 0) {
			return -EINVAL;
		}

		ret = of_property_read_u32_array(
			np, "jeita_iterm_cfg_second_supplier",
			(u32 *)info->jeita_iterm_cfg, total_length);
		if (ret) {
			return ret;
		}
	...
	case BATTERY_SUPPLIER_NVT:
		total_length = of_property_count_elems_of_size(
			np, "jeita_iterm_cfg", sizeof(u32));
		if (total_length < 0) {
			return -EINVAL;
		}

		ret = of_property_read_u32_array(np, "jeita_iterm_cfg",
						 (u32 *)info->jeita_iterm_cfg,
						 total_length);
		if (ret) {
			return ret;
		}
...
static int parse_step_charge_config(struct mtk_charger *info, bool force_update)
{
	...
	if (info->bms_psy) {
		ret = power_supply_get_property(
			info->bms_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, &pval);
		if (ret)
			pval.intval = 0;
		else
	...
	if (update || force_update) {
		if (low_cycle) {
			total_length = of_property_count_elems_of_size(
				np, "step_chg_cfg_low_cycle", sizeof(u32));
			if (total_length < 0) {
				return -EINVAL;
			}

			ret = of_property_read_u32_array(
				np, "step_chg_cfg_low_cycle",
				(u32 *)info->step_chg_cfg, total_length);
			if (ret) {
				return ret;
			}
		} else if (middle_cycle) {
			total_length = of_property_count_elems_of_size(
				np, "step_chg_cfg_middle_cycle", sizeof(u32));
			if (total_length < 0) {
				return -EINVAL;
			}

			ret = of_property_read_u32_array(
				np, "step_chg_cfg_middle_cycle",
				(u32 *)info->step_chg_cfg, total_length);
			if (ret) {
				return ret;
			}
			...
			total_length = of_property_count_elems_of_size(
				np, "step_chg_cfg_high_cycle", sizeof(u32));
			if (total_length < 0) {
				return -EINVAL;
			}

			ret = of_property_read_u32_array(
				np, "step_chg_cfg_high_cycle",
				(u32 *)info->step_chg_cfg, total_length);
			if (ret) {
				return ret;
			}
...
static void monitor_sw_cv(struct mtk_charger *info)
{
	...
	if (info->step_chg_index[0] > info->step_chg_index[1] &&
	    (info->step_chg_cfg[info->step_chg_index[0]].value !=
	     info->step_chg_cfg[info->step_chg_index[1]].value)) {
		info->sw_cv_count = 0;
		info->sw_cv = info->step_chg_cfg[info->step_chg_index[0]]
				      .low_threshold +
			      info->step_forward_hyst;
	}
	...
	if (info->sw_cv || info->suspend_recovery) {
		...
		if ((-ibat) <= info->step_chg_fcc) {
			...
			if (info->sw_cv_count >= SW_CV_COUNT) {
				info->sw_cv = 0;
				info->sw_cv_count = 0;
				vote(info->fcc_votable, STEP_CHARGE_VOTER, true,
				     info->step_chg_fcc);
			}
		} else {
			info->sw_cv_count = 0;
...
static void monitor_jeita_descent(struct mtk_charger *info)
{
	...
	if (current_fcc != info->jeita_chg_fcc) {
		if (current_fcc >= info->jeita_chg_fcc + JEITA_FCC_DESCENT_STEP)
			vote(info->fcc_votable, JEITA_CHARGE_VOTER, true,
			     current_fcc - JEITA_FCC_DESCENT_STEP);
		else if (current_fcc >=
			 info->jeita_chg_fcc - JEITA_FCC_DESCENT_STEP)
			vote(info->fcc_votable, JEITA_CHARGE_VOTER, true,
			     info->jeita_chg_fcc);
		else
			vote(info->fcc_votable, JEITA_CHARGE_VOTER, true,
...
static void monitor_usb_otg_burn(struct work_struct *work)
{
	...
	} else if ((info->typec_otg_burn &&
		    type_temp <= (TYPEC_BURN_TEMP - TYPEC_BURN_HYST)) ||
		   (pmic_vbus < 1000 && info->otg_enable &&
		    (type_temp <= TYPEC_BURN_TEMP - TYPEC_BURN_HYST))) {
		info->typec_otg_burn = false;
		usbotg_enable_otg(info, true);
	}
	schedule_delayed_work(&info->usb_otg_monitor_work,
			      msecs_to_jiffies(otg_monitor_delay_time));
}
...
static void rust_detection_work_func(struct work_struct *work)
{
	...
	if (info->lpd_res[1] < 40 || info->lpd_res[2] < 40 ||
	    info->lpd_res[3] < 40 || info->lpd_res[4] < 40) {
...
out:
	schedule_delayed_work(&info->rust_detection_work,
			      msecs_to_jiffies(rust_det_interval));
}
...
static void generate_xm_lpd_uevent(struct mtk_charger *info)
{
	...
	sprintf(uevent_string + 28,
		"LPD_DP_RES=%d\nLPD_DM_RES=%d\nLPD_SBU1_RES=%d\nLPD_SBU2_RES=%d\n",
		info->lpd_res[1], info->lpd_res[2], info->lpd_res[3],
		info->lpd_res[4]);
...
static void monitor_typec_burn(struct mtk_charger *info)
{
	...
	if (type_temp >= TYPEC_BURN_TEMP && !info->typec_burn_status &&
	    !info->otg_enable) {
		info->typec_burn = true;
		while (retry_count) {
			if (info->cp_master && info->cp_slave) {
				charger_dev_is_enabled(info->cp_master,
						       &cp_master_enable);
				charger_dev_is_enabled(info->cp_slave,
						       &cp_slave_enable);
			}
		...
		if (info->real_type == XMUSB350_TYPE_HVCHG)
			charger_set_dpdm_voltage(info, 0, 0);
			 ...
			 info->real_type == XMUSB350_TYPE_HVDCP_35_18 ||
			 info->real_type == XMUSB350_TYPE_HVDCP_35_27 ||
			 info->real_type == XMUSB350_TYPE_HVDCP_3_18 ||
			 info->real_type == XMUSB350_TYPE_HVDCP_3_27)
			charger_dev_select_qc_mode(info->usb350_dev,
						   QC_MODE_QC2_5);
	...
	} else if (info->typec_burn &&
		   type_temp <= TYPEC_BURN_TEMP - TYPEC_BURN_HYST &&
		   info->typec_burn_status) {
...
static void handle_jeita_charge(struct mtk_charger *info)
{
	...
	get_index(info->jeita_fv_cfg, info->jeita_fallback_hyst,
		  info->jeita_forward_hyst, info->temp_now,
		  info->jeita_chg_index, false, STEP_JEITA_TUPLE_COUNT);
	if (info->jeita_chg_index[0] == 0 || info->product_name == ZIRCON) {
		vote(info->fv_votable, JEITA_CHARGE_VOTER, true,
		     info->jeita_fv_cfg[info->jeita_chg_index[0]].value -
			     info->diff_fv_val -
			     (info->product_name == ZIRCON ? 0 : 10));
	} else {
		vote(info->fv_votable, JEITA_CHARGE_VOTER, true,
		     info->jeita_fv_cfg[info->jeita_chg_index[0]].value -
			     info->diff_fv_val);
	...
	if (info->jeita_chg_index[0] == STEP_JEITA_TUPLE_COUNT - 2 &&
	    info->jeita_chg_index[1] == STEP_JEITA_TUPLE_COUNT - 1) {
		info->high_temp_rec_soc = info->soc;
	}

	if (jeita_vbat_low) {
		if (info->vbat_now <
		    (info->jeita_fcc_cfg[info->jeita_chg_index[0]]
			     .extra_threshold +
		     50)) {
			info->jeita_chg_fcc =
				info->jeita_fcc_cfg[info->jeita_chg_index[0]]
...
{
	get_index(info->step_chg_cfg, info->step_fallback_hyst,
		  info->step_forward_hyst, info->vbat_now, info->step_chg_index,
		  false, STEP_CHG_TUPLE_COUNT);

	if (info->step_chg_index[0] == STEP_CHG_TUPLE_COUNT - 1)
		info->step_chg_fcc =
			info->step_chg_cfg[info->step_chg_index[0]].value + 100;
	else
		info->step_chg_fcc =
			info->step_chg_cfg[info->step_chg_index[0]].value;
...
{
	if (info->ffc_enable)
		info->iterm = info->jeita_iterm_cfg[info->jeita_chg_index[0]]
				      .value_ffc;
	else
		info->iterm =
			info->jeita_iterm_cfg[info->jeita_chg_index[0]].value;
...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	if ((!info->recharge && info->entry_soc <= info->ffc_high_soc &&
	     is_between(info->ffc_low_tbat, info->ffc_high_tbat,
			info->temp_now) &&
	     info->high_temp_rec_soc <= 95 &&
	     (info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO &&
	      info->pd_adapter->verifed) &&
	     (info->thermal_level <= 14)) ||
	    info->suspend_recovery)
		info->ffc_enable = true;
	else
		info->ffc_enable = false;

	if (info->product_name == ZIRCON && info->is_charging &&
	    info->soc > info->ffc_high_soc)
		info->ffc_enable = false;

	if (info->product_name == COROT) {
		if (is_between(info->ffc_medium_tbat, info->ffc_high_tbat,
			       info->temp_now)) {
			iterm = min(info->iterm_warm, 100);
			if (info->cycle_count <= 100) {
				reduceMv_flag = true;

				charger_dev_is_enabled(info->chg1_dev,
						       &chg1_en);
				if (!info->cycle_count_reduceXMv && chg1_en &&
				    info->vbat_now > 4450) {
	...
	if (info->is_charging) {
		...
		} else if (XMUSB350_TYPE_SDP <= info->real_type &&
	...
	if (info->ffc_enable) {
		vote(info->fv_votable, FFC_VOTER, true,
		     info->fv_ffc - info->diff_fv_val - accuracy_temp);
		...
		vote(info->fv_votable, FFC_VOTER, true,
		     reduceMv_flag ?
			     info->fv - 10 - info->cycle_count_reduceXMv :
			     info->fv - info->diff_fv_val - accuracy_temp);
...
{
	static int full_count = 0, recharge_count = 0, iterm = 0,
		   threshold_mv = 0, recharge_voltage = 0;
	int iterm_effective = get_effective_result(info->iterm_votable);
	...
	if (info->charge_full) {
		...
		if (recharge_count >= 15 ||
		    (info->product_name == COROT && rsoc < 97 &&
		     info->temp_now < 460 && info->temp_now > 0) ||
		    (info->product_name == ZIRCON && rsoc <= 95 &&
		     info->temp_now < 460 && info->temp_now > 0)) {
		...
		if (info->temp_now > 0)
			...
			threshold_mv = 100;
		if ((info->soc == 100 &&
		     (info->vbat_now >= fv_effective - threshold_mv) &&
		     (((-info->current_now <= iterm) && info->fg_full) ||
		      ((-info->current_now <= iterm_effective + 100) &&
		       info->bbc_charge_done))) ||
		    (info->soc == 100 &&
		     (info->vbat_now >= fv_effective - 30) &&
		     (-info->current_now <= iterm) &&
		     (info->temp_now >= 480)) ||
		    (info->product_name == ZIRCON &&
		     (info->vbat_now >= fv_effective - threshold_mv) &&
		     (-info->current_now <= iterm) &&
		     info->jeita_chg_index[0] == JEITA_NOT_CARE_FG_FULL_INDEX))
			full_count++;
		else
			full_count = 0;
		...
		if (full_count >= 6) {
			...
			info->warm_term = true;
			adapter_dev_set_cap_xm(info->pd_adapter, MTK_PD_APDO,
					       5000, 3000);
		...
		if (info->soc < 100 && info->bbc_charge_done &&
		    info->temp_now < 475 && info->temp_now > 0 &&
		    info->product_name != ZIRCON) {
...
static void check_charge_data(struct mtk_charger *info)
{
	...
	if (info->chg_psy) {
		ret = power_supply_get_property(
			info->chg_psy, POWER_SUPPLY_PROP_STATUS, &pval);
	...
	ret = power_supply_get_property(info->bms_psy, POWER_SUPPLY_PROP_TEMP,
					&pval);
	...
	if (chg1_en && !info->ffc_enable && !info->ov_check_only_once &&
	    info->vbat_now >= (info->fv - 10)) {
	...
	if (info->product_name == COROT) {
		if (info->cycle_count > 800 &&
		    is_between(MIN_FFC_JEITA_CHG_INDEX, MAX_FFC_JEITA_CHG_INDEX,
			       info->jeita_chg_index[0])) {
			info->diff_fv_val = 40;
		} else if (info->cycle_count > 100 &&
			   is_between(MIN_FFC_JEITA_CHG_INDEX,
				      MAX_FFC_JEITA_CHG_INDEX,
				      info->jeita_chg_index[0])) {
			info->diff_fv_val = 20;
		}
	}
		...
		if (info->ffc_enable) {
			...
			if (0 <= info->cycle_count && info->cycle_count <= 100)
				info->diff_fv_val = 20;
				 ...
				 info->cycle_count <= 300)
				info->diff_fv_val = 30;
			else if (301 <= info->cycle_count &&
				 info->cycle_count <= 800)
				info->diff_fv_val = 40;
			else if (info->cycle_count >= 801)
				info->diff_fv_val = 60;
	...
	if (info->smart_batt_reduceXMv) {
		info->diff_fv_val =
			max(info->diff_fv_val, info->smart_batt_reduceXMv);
	}
}
...
static void monitor_smart_chg(struct mtk_charger *info)
{
	...
	if (info->smart_chg[SMART_CHG_NAVIGATION].en_ret &&
		...
		charger_dev_enable(info->chg1_dev, false);
	} else if ((!info->smart_chg[SMART_CHG_NAVIGATION].en_ret &&
		    last_navig_stat) ||
		   (info->soc <=
			    info->smart_chg[SMART_CHG_NAVIGATION].func_val -
				    5 &&
		    info->smart_chg[SMART_CHG_NAVIGATION].active_status)) {
	...
	if (info->smart_chg[SMART_CHG_BATT_LOW_FAST].active_status == true &&
	    screen_status_last == DISPLAY_SCREEN_OFF &&
	    info->screen_status == DISPLAY_SCREEN_ON) {
		last_change_time = ktime_get();
	} else if (info->smart_chg[SMART_CHG_BATT_LOW_FAST].active_status ==
			   false &&
		   screen_status_last == DISPLAY_SCREEN_ON &&
		   info->screen_status == DISPLAY_SCREEN_OFF &&
		   info->soc < LOW_FAST_EXIT_SOC) {
		last_change_time = -1;
		screen_off_soc_low = true;
		screen_on_timeout = false;
	} else if (screen_status_last == DISPLAY_SCREEN_OFF &&
		   info->screen_status == DISPLAY_SCREEN_OFF) {
		last_change_time = -1;
	...
	if (last_change_time >= 0)
		screen_on_timeout =
			(ktime_ms_delta(ktime_get(), last_change_time) >
			 LOW_FAST_SCREEN_TIMEOUT_MS) ?
				true :
				false;
	...
	if (info->low_fast_in_process &&
	    info->smart_chg[SMART_CHG_BATT_LOW_FAST].active_status == false &&
	    (screen_off_soc_low ||
	     (!board_temp_is_high && info->soc < LOW_FAST_NOT_RESTART_SOC))) {
		info->smart_chg[SMART_CHG_BATT_LOW_FAST].active_status = true;
		vote(info->fcc_votable, SIC_VOTER, false, 0);
	}
	...
	if (info->smart_chg[SMART_CHG_BATT_LOW_FAST].active_status == true) {
		if (last_sic_current != info->sic_current) {
			info->sic_thermal_level =
				get_level_from_pd_thermal(info);
			sic_current_incre = info->sic_current +
					    info->low_fast_current_incre
						    [info->sic_thermal_level];
			sic_current_incre =
				(sic_current_incre >= MAX_THERMAL_FCC) ?
					MAX_THERMAL_FCC :
					sic_current_incre;
			vote(info->fcc_votable, SIC_INCRE_VOTER, true,
			     sic_current_incre);
		}
	}
...
static void monitor_slave_connector(struct mtk_charger *info)
{
	...
	if (info->product_name == COROT) {
		...
		if (val >= 1200 && !info->mt6363_auxadc4_enable) {
			vote(info->fcc_votable, SLAVE_CON_ERR_VOTER, true,
			     2000);
		...
		case BATT_DISCONNECT:
			vote(info->fcc_votable, SLAVE_CON_ERR_VOTER, true,
			     2000);
			vote(info->icl_votable, SLAVE_CON_ERR_VOTER, true,
			     2000);
...
static void monitor_isc_and_soa_charge(struct mtk_charger *info)
{
	...
	if ((batt_isc_alert_level == 3) || batt_soa_alert_level) {
		info->diff_fv_val =
			info->diff_fv_val > 15 ? info->diff_fv_val : 15;
...
static void charge_monitor_func(struct work_struct *work)
{
	...
	schedule_delayed_work(&info->charge_monitor_work,
			      msecs_to_jiffies(FCC_DESCENT_DELAY));
}

static void delay_reset_full_flag_work_func(struct work_struct *work)
{
	struct mtk_charger *info = container_of(
		work, struct mtk_charger, delay_reset_full_flag_work.work);
...
void reset_step_jeita_charge(struct mtk_charger *info)
{
	...
	get_index(info->jeita_fv_cfg, info->jeita_fallback_hyst,
		  info->jeita_forward_hyst, info->temp_now,
		  info->jeita_chg_index, true, STEP_JEITA_TUPLE_COUNT);
	if (info->vbat_now <
	    info->jeita_fcc_cfg[info->jeita_chg_index[0]].extra_threshold)
		info->jeita_chg_fcc =
			info->jeita_fcc_cfg[info->jeita_chg_index[0]].low_value;
	else
		info->jeita_chg_fcc =
			info->jeita_fcc_cfg[info->jeita_chg_index[0]].high_value;
...
int step_jeita_init(struct mtk_charger *info, struct device *dev)
{
	...
	total_length = of_property_count_elems_of_size(np, "thermal_limit_dcp",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_dcp",
					 (u32 *)(info->thermal_limit[0]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		if (info->thermal_limit[0][i] > MAX_THERMAL_FCC ||
		    info->thermal_limit[0][i] < MIN_THERMAL_FCC) {
			return -1;
		}
		if (i != 0) {
			if (info->thermal_limit[0][i] >
			    info->thermal_limit[0][i - 1]) {
				return -1;
			}
		}
	}

	total_length = of_property_count_elems_of_size(np, "thermal_limit_qc2",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_qc2",
					 (u32 *)(info->thermal_limit[1]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		if (info->thermal_limit[1][i] > MAX_THERMAL_FCC ||
		    info->thermal_limit[1][i] < MIN_THERMAL_FCC) {
			return -1;
		}
		if (i != 0) {
			if (info->thermal_limit[1][i] >
			    info->thermal_limit[1][i - 1]) {
				return -1;
			}
		}
	}
	...
	total_length = of_property_count_elems_of_size(
		np, "thermal_limit_qc3_18w", sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_qc3_18w",
					 (u32 *)(info->thermal_limit[2]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		if (info->thermal_limit[2][i] > MAX_THERMAL_FCC ||
		    info->thermal_limit[2][i] < MIN_THERMAL_FCC) {
			return -1;
		}
		if (i != 0) {
			if (info->thermal_limit[2][i] >
			    info->thermal_limit[2][i - 1]) {
				return -1;
			}
		}
	}
	...
	total_length = of_property_count_elems_of_size(
		np, "thermal_limit_qc3_27w", sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_qc3_27w",
					 (u32 *)(info->thermal_limit[3]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		if (info->thermal_limit[3][i] > MAX_THERMAL_FCC ||
		    info->thermal_limit[3][i] < MIN_THERMAL_FCC) {
			return -1;
		}
		if (i != 0) {
			if (info->thermal_limit[3][i] >
			    info->thermal_limit[3][i - 1]) {
				return -1;
			}
		}
	}

	total_length = of_property_count_elems_of_size(np, "thermal_limit_qc35",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_qc35",
					 (u32 *)(info->thermal_limit[4]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		if (info->thermal_limit[4][i] > MAX_THERMAL_FCC ||
		    info->thermal_limit[4][i] < MIN_THERMAL_FCC) {
			return -1;
		}
		if (i != 0) {
			if (info->thermal_limit[4][i] >
			    info->thermal_limit[4][i - 1]) {
				return -1;
			}
		}
	}

	total_length = of_property_count_elems_of_size(np, "thermal_limit_pd",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}

	ret = of_property_read_u32_array(np, "thermal_limit_pd",
					 (u32 *)(info->thermal_limit[5]),
					 total_length);
	if (ret) {
		return ret;
	}
	...
	for (i = 0; i < THERMAL_LIMIT_COUNT; i++) {
		...
		if (i != 0) {
			if (info->thermal_limit[5][i] >
			    info->thermal_limit[5][i - 1]) {
				return -1;
			}
		}
	}
	...
	if (info->product_name == ZIRCON) {
		total_length = of_property_count_elems_of_size(
			np, "smart_chg_lowfast_increment", sizeof(u32));
		if (total_length < 0) {
			return -EINVAL;
		}

		ret = of_property_read_u32_array(
			np, "smart_chg_lowfast_increment",
			(u32 *)(info->low_fast_current_incre), total_length);
		if (ret) {
			return ret;
		}
	}
	...
	total_length = of_property_count_elems_of_size(np, "jeita_fcc_cfg",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}
	...
	ret = of_property_read_u32_array(
		np, "jeita_fcc_cfg", (u32 *)info->jeita_fcc_cfg, total_length);
	if (ret) {
		return ret;
	}

	total_length = of_property_count_elems_of_size(np, "jeita_fv_cfg",
						       sizeof(u32));
	if (total_length < 0) {
		return -EINVAL;
	}
	...
	ret = of_property_read_u32_array(
		np, "jeita_fv_cfg", (u32 *)info->jeita_fv_cfg, total_length);
	if (ret) {
		return ret;
	}

	ret = of_property_read_u32(np, "step_fallback_hyst",
				   &info->step_fallback_hyst);
	if (ret) {
		return ret;
	}

	ret = of_property_read_u32(np, "step_forward_hyst",
				   &info->step_forward_hyst);
	if (ret) {
		return ret;
	}

	ret = of_property_read_u32(np, "jeita_fallback_hyst",
				   &info->jeita_fallback_hyst);
	if (ret) {
		return ret;
	}

	ret = of_property_read_u32(np, "jeita_forward_hyst",
				   &info->jeita_forward_hyst);
	if (ret) {
		return ret;
	}
	...
	if (IS_ERR(info->vbus_contral))
		...
		info->mt6373_moscon1_control =
			of_property_read_bool(np, "mt6373_moscon1_control");

	info->sic_support = of_property_read_bool(np, "sic_support");

	if (info->product_name == COROT) {
		info->mt6363_auxadc4 =
			devm_iio_channel_get(dev, "mt6363_auxadc4");
		if (IS_ERR(info->mt6363_auxadc4)) {
			info->mt6363_auxadc4 = NULL;
		}
	} else if (info->product_name == ZIRCON) {
		info->slave_connect_check_gpio =
			of_get_named_gpio(np, "slave_connect_check_gpio", 0);
		if (!gpio_is_valid(info->slave_connect_check_gpio)) {
			return -ENODEV;
		}
	...
	INIT_DELAYED_WORK(&info->delay_reset_full_flag_work,
			  delay_reset_full_flag_work_func);
#if defined(CONFIG_RUST_DETECTION)
