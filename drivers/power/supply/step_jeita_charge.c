...
static void monitor_usb_otg_burn(struct work_struct *work)
{
	...
	chr_err("%s:get typec temp =%d otg_monitor_delay_time = %d\n", __func__,
		type_temp, otg_monitor_delay_time);
...
static void generate_xm_lpd_uevent(struct mtk_charger *info)
{
	...
	char *envp[5] = {
		NULL
	}; //the length of array need adjust when uevent number increase
...
static void handle_jeita_charge(struct mtk_charger *info)
{
	...
	if (is_between(info->jeita_fcc_cfg[0].low_threshold,
		       info->jeita_fcc_cfg[STEP_JEITA_TUPLE_COUNT - 1]
			       .high_threshold,
		       info->temp_now) &&
	    !info->input_suspend && !info->typec_burn &&
	    info->bms_i2c_error_count < 10)
		chr_err("handle_jeita_charge index = %d,jeita_chg_fcc = %d",
			info->jeita_chg_index[0], info->jeita_chg_fcc);
...
static void check_full_recharge(struct mtk_charger *info)
{
	static int full_count = 0, recharge_count = 0, iterm = 0,
		   threshold_mv = 0, real_full = 0, recharge_voltage = 0;
	int iterm_effective = get_effective_result(info->iterm_votable);
	...
	if (info->charge_full) {
		...
		if (recharge_count >= 15 ||
		    (rsoc < 97 && info->temp_now < 460 && info->temp_now > 0)) {
		...
		if ((info->soc == 100 &&
		    ...
		    (info->soc == 100 &&
		     ...
		     (-info->current_now <= iterm) && (info->temp_now >= 480)))
			full_count++;
		else
			full_count = 0;
...
static void check_charge_data(struct mtk_charger *info)
{
	...
	chr_err("[CHARGE_LOOP] TYPE = [%d %d %d %d], BMS = [%d %d %d %d], FULL = [%d %d %d %d %d], thermal_level=%d, FFC = %d, sw_cv=%d, gauge_authentic=%d,warm_term=%d\n",
		info->pd_type, info->pd_adapter->verifed, info->real_type,
		info->qc3_type, info->soc, info->vbat_now, info->current_now,
		info->temp_now, info->bbc_charge_enable, info->charge_full,
		info->fg_full, info->bbc_charge_done, info->recharge,
		info->thermal_level, info->ffc_enable, info->sw_cv,
		info->gauge_authentic, info->warm_term);
...
static void monitor_isc_and_soa_charge(
	struct mtk_charger *info) //NFG1000A & NFG1000B支持该功能
{
	int ret = 0;
	...
	chr_err("[%s]:isc_alert_level=%d, soa_alert_level=%d\n", __func__,
		batt_isc_alert_level, batt_soa_alert_level);
	...
	if (update || farce_update) {
		...
		for (i = 0; i < STEP_JEITA_TUPLE_COUNT; i++)
			chr_err("%d STEP %d %d %d\n", info->cycle_count,
				...
				info->step_chg_cfg[i].high_threshold,
				info->step_chg_cfg[i].value);
	}
...
int step_jeita_init(struct mtk_charger *info, struct device *dev)
{
	...
	for (i = 0; i < STEP_JEITA_TUPLE_COUNT; i++)
		chr_info("JEITA_FCC %d %d %d %d %d\n",
			 info->jeita_fcc_cfg[i].low_threshold,
			 info->jeita_fcc_cfg[i].high_threshold,
			 info->jeita_fcc_cfg[i].extra_threshold,
			 info->jeita_fcc_cfg[i].low_value,
	...
	for (i = 0; i < STEP_JEITA_TUPLE_COUNT; i++)
		chr_info("JEITA_FV %d %d %d\n",
			 info->jeita_fv_cfg[i].low_threshold,
			 info->jeita_fv_cfg[i].high_threshold,
			 info->jeita_fv_cfg[i].value);
