...
int sc8561_enable_adc(struct sc8561_device *bq, bool enable)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_15, SC8561_ADC_EN_MASK,
				 val);
	return ret;
}
...
int sc8561_enable_charge(struct sc8561_device *bq, bool enable)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_0B, SC8561_CHG_EN_MASK,
				 val);
	return ret;
}
...
static int sc8561_set_batovp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_01, SC8561_BAT_OVP_MASK,
				 val);
	return ret;
}
/* SC8561_BAT_OCP */
...
static int sc8561_set_batocp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_02, SC8561_BAT_OCP_MASK,
				 val);
	return ret;
}
/* SC8561_USB_OVP */
static int sc8561_set_usbovp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_03, SC8561_USB_OVP_MASK,
				 val);
	return ret;
}
...
static int sc8561_set_wpcovp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_04, SC8561_WPC_OVP_MASK,
				 val);
	return ret;
}
/* SC8561_BUS_OVP */
static int sc8561_set_busovp_th(struct sc8561_device *bq, int threshold)
{
	...
	if (bq->work_mode == SC8561_FORWARD_4_1_CHARGER_MODE ||
		...
		if (threshold < 14000)
			...
			threshold = 22000;
		val = (threshold - SC8561_BUS_OVP_41MODE_BASE) /
		...
		if (threshold < 7000)
			...
			threshold = 13300;
		val = (threshold - SC8561_BUS_OVP_21MODE_BASE) /
		...
		if (threshold < 3500)
			...
			threshold = 5500;
		val = (threshold - SC8561_BUS_OVP_11MODE_BASE) /
		      SC8561_BUS_OVP_11MODE_LSB;
	}
	bq_info("sc8561_set_busovp_th= %d", threshold);
	val <<= SC8561_BUS_OVP_SHIFT;
	ret = regmap_update_bits(bq->regmap, SC8561_REG_05, SC8561_BUS_OVP_MASK,
				 val);
	return ret;
}
/* SC8561_OUT_OVP */
static int sc8561_set_outovp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_05, SC8561_OUT_OVP_MASK,
				 val);
	return ret;
}
/* SC8561_BUS_OCP */
...
static int sc8561_set_busocp_th(struct sc8561_device *bq, int threshold)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_06, SC8561_BUS_OCP_MASK,
				 val);
	return ret;
}
/* SC8561_BUS_UCP */
...
int sc8561_get_adc_data(struct sc8561_device *bq, int channel, u32 *result)
{
	...
	if (channel >= ADC_MAX_NUM)
		return -1;
	ret = regmap_read(bq->regmap, SC8561_ADC_REG_BASE + (channel << 1),
			  &val_h);
	ret = regmap_read(bq->regmap, SC8561_ADC_REG_BASE + (channel << 1) + 1,
			  &val_l);
	if (ret < 0)
		return ret;
	val = (val_h << 8) | val_l;
...
static int sc8561_set_adc_scan(struct sc8561_device *bq, int channel,
			       bool enable)
{
	int ret = 0;
	u8 reg;
...
int sc8561_set_reg_reset(struct sc8561_device *bq)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_0E, SC8561_REG_RST_MASK,
				 val);
	return ret;
}
/* SC8561_MODE */
int sc8561_set_operation_mode(struct sc8561_device *bq, int operation_mode)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_0E, SC8561_MODE_MASK,
				 val);
	return ret;
}
...
static int sc8561_set_ss_timeout(struct sc8561_device *bq, int timeout)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_0D,
				 SC8561_SS_TIMEOUT_SET_MASK, val);
	return ret;
}
...
static int sc8561_set_sync(struct sc8561_device *bq, u8 date)
{
	...
	ret = regmap_update_bits(bq->regmap, SC8561_REG_0C, SC8561_SYNC_MASK,
				 val);
	return ret;
}
...
static int sc8561_dump_important_regs(struct sc8561_device *bq,
				      union cp_propval *val)
{
	...
	for (i = 0; i < reg_num; i++) {
		...
		len = scnprintf(buf_tmp + strlen(buf_tmp), PAGE_SIZE - idx,
				"[0x%02X]=0x%02X,", sc8561_reg_list[i],
				data[i]);
		idx += len;
		...
		if (((i + 1) % 8 == 0) || ((i + 1) == reg_num)) {
			...
			if (val) /* print value of regs in adb */
				len_sysfs = scnprintf(
					val->strval + strlen(val->strval),
	...
	if ((data[9] & 0x02) == 0) {
		bq_info("cp switching stop, enter abnormal charging judge\n");
...
static int sc8561_init_protection(struct sc8561_device *cp,
...
static int sc8561_init_device(struct sc8561_device *cp)
{
	...
	/*
	If working mode of OVPGATE/WPCGATE needs to be switched from
	Auto mode to Manual mode, the OVPGATE_EN and WPCGATE_EN bit
	must be set correctly to avoid input source voltage drops due
	to shutdown of gate-driver.
	*/
	//	ret = sc8561_set_wdt(cp, 0);
	ret = sc8561_enable_acdrv_manual(cp, false); //ac drive manual mode
...
static int ops_cp_dump_register(struct charger_device *chg_dev)
{
	...
	if (ret)
		bq_err("%s failed dump registers, ret=%d \n", bq->log_tag, ret);
...
static int ops_cp_get_charge_enable(struct charger_device *chg_dev,
				    bool *enabled)
{
	struct sc8561_device *bq = charger_get_data(chg_dev);
	int ret = 0;
...
static int ops_cp_get_mode(struct charger_device *chg_dev, int *mode)
{
	...
	if (bq->chip_vendor == SC8561) {
		...
		if (ret)
			bq_err("%s failed to get div_mode\n", bq->log_tag);
	} else
		ret = -1;
	return ret;
}
...
static int ops_cp_is_bypass_enabled(struct charger_device *chg_dev,
				    bool *enabled)
{
	*enabled = false;
	return 0;
...
static int ops_cp_get_bypass_support(struct charger_device *chg_dev,
				     bool *enabled)
{
	*enabled = 1;
	chr_err("%s %d\n", __func__, *enabled);
...
static void sc8561_irq_handler(struct work_struct *work)
{
	...
	//sc8561_dump_important_regs(bq,NULL);
...
static int sc8561_init_irq(struct sc8561_device *bq)
{
	...
	ret = request_irq(bq->irq, sc8561_interrupt,
...
static int sc8561_register_charger(struct sc8561_device *bq, int driver_data)
{
	switch (driver_data) {
	case SC8561_SLAVE:
		bq->chg_dev = charger_device_register("cp_slave", bq->dev, bq,
						      &sc8561_chg_ops,
						      &sc8561_slave_chg_props);
		if (bq->chip_vendor == SC8561) {
		...
		break;
	case SC8561_MASTER:
		bq->chg_dev = charger_device_register("cp_master", bq->dev, bq,
						      &sc8561_chg_ops,
						      &sc8561_master_chg_props);
		if (bq->chip_vendor == SC8561) {
			     ...
			     char *buf)
{
	...
	/* handle attr which will output a string */
	case CP_PROP_DUMP_REG:
		count = scnprintf(buf, PAGE_SIZE, "%s\n", val.strval);
		break;

	/* handle attr which will output an int or uint */
	case CP_PROP_VBUS ... CP_PROP_ADC_ENABLE:
...
static int sc8561_power_supply_init(struct sc8561_device *bq,
				    struct device *dev, int driver_data)
{
	...
	bq->cp_psy =
		devm_power_supply_register(bq->dev, &bq->psy_desc, &psy_cfg);
	if (IS_ERR(bq->cp_psy))
		return -EINVAL;
