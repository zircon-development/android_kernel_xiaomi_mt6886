...
struct mtk_cp_sysfs_field_info {
	...
	int (*set)(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		   int val);
	int (*get)(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		   int *val);
};
...
static const char *const adc_channel_name[] = {
	[ADC_IBUS] = "IBUS", [ADC_VBUS] = "VBUS", [ADC_VAC] = "VAC",
	[ADC_VOUT] = "VOUT", [ADC_VBAT] = "VBAT", [ADC_IBAT] = "IBAT",
	[ADC_TDIE] = "TDIE",
};

static struct adc_desc adc_desc_table[] = {
	[ADC_IBUS] = { SC8551_ADC_IBUS_REG, SC8551_ADC_IBUS_MASK,
		       SC8551_ADC_IBUS_LSB, SC8551_ADC_IBUS_RATE },
	[ADC_VBUS] = { SC8551_ADC_VBUS_REG, SC8551_ADC_VBUS_MASK,
		       SC8551_ADC_VBUS_LSB, SC8551_ADC_VBUS_RATE },
	[ADC_VAC] = { SC8551_ADC_VAC_REG, SC8551_ADC_VAC_MASK,
		      SC8551_ADC_VAC_LSB, SC8551_ADC_VAC_RATE },
	[ADC_VOUT] = { SC8551_ADC_VOUT_REG, SC8551_ADC_VOUT_MASK,
		       SC8551_ADC_VOUT_LSB, SC8551_ADC_VOUT_RATE },
	[ADC_VBAT] = { SC8551_ADC_VBAT_REG, SC8551_ADC_VBAT_MASK,
		       SC8551_ADC_VBAT_LSB, SC8551_ADC_VBAT_RATE },
	[ADC_IBAT] = { SC8551_ADC_IBAT_REG, SC8551_ADC_IBAT_MASK,
		       SC8551_ADC_IBAT_LSB, SC8551_ADC_IBAT_RATE },
	[ADC_TDIE] = { SC8551_ADC_TDIE_REG, SC8551_ADC_TDIE_MASK,
		       SC8551_ADC_TDIE_LSB, SC8551_ADC_TDIE_RATE },
};
...
static int sc8551_enable_adc(struct sc8551 *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_CTRL_REG,
				 SC8551_ENABLE_ADC_BIT,
				 enable ? SC8551_ENABLE_ADC_BIT : 0);
...
static int sc8551_set_bus_ovp(struct sc8551 *chip, int value)
{
	...
	data = ((value - SC8551_BUS_OVP_BASE) / SC8551_BUS_OVP_LSB)
	       << SC8551_BUS_OVP_SHIFT;

	ret = regmap_update_bits(chip->regmap, SC8551_BUS_OVP_REG,
...
static int sc8551_set_ac_ovp(struct sc8551 *chip, int value)
{
	...
	data = ((value - SC8551_VAC_OVP_BASE) / SC8551_VAC_OVP_LSB)
	       << SC8551_VAC_OVP_SHIFT;

	ret = regmap_update_bits(chip->regmap, SC8551_AC_PROTECT_REG,
...
static int sc8551_enable_bypass(struct sc8551 *chip, int enable)
{
	...
	ret = regmap_update_bits(chip->regmap, SC8551_BYPASS_REG,
				 SC8551_ENABLE_BYPASS_BIT,
				 enable ? SC8551_ENABLE_BYPASS_BIT : 0);
...
static int sc8551_enable_charge(struct sc8551 *chip, bool enable)
{
	...
	ret = regmap_update_bits(chip->regmap, SC8551_CHG_CTRL_REG,
				 SC8551_ENABLE_CHG_BIT,
				 enable ? SC8551_ENABLE_CHG_BIT : 0);
...
static int sc8551_read_adc(struct sc8551 *chip, int channel, int *value)
{
	...
	ret = regmap_raw_read(chip->regmap, adc_desc_table[channel].reg, data,
			      2);
	if (ret) {
	...
	if (channel == ADC_TDIE) {
		...
		tchg_result = abs_value * adc_desc_table[ADC_TDIE].step /
			      adc_desc_table[ADC_TDIE].rate;
		if (data[0] & 0x80)
			*value = -tchg_result;
		else
			*value = tchg_result;
		...
		*value = ((((data[0] << 8) + data[1]) &
			   adc_desc_table[channel].mask) *
			  adc_desc_table[channel].step) /
			 adc_desc_table[channel].rate;
	}
...
static int sc8551_set_bus_ocp(struct sc8551 *chip, int value)
{
	...
	data = ((value - SC8551_BUS_OCP_BASE) / SC8551_BUS_OCP_LSB)
	       << SC8551_BUS_OCP_SHIFT;

	ret = regmap_update_bits(chip->regmap, SC8551_BUS_OCP_UCP_REG,
				 SC8551_BUS_OCP_MASK, data);
	if (ret)
...
static int sc8551_set_bat_ovp(struct sc8551 *chip, int value)
{
	...
	data = ((value - SC8551_BAT_OVP_BASE) / SC8551_BAT_OVP_LSB)
	       << SC8551_BAT_OVP_SHIFT;

	ret = regmap_update_bits(chip->regmap, SC8551_BAT_OVP_REG,
...
static int sc8551_set_bat_ocp(struct sc8551 *chip, int value)
{
	...
	data = ((value - SC8551_BAT_OCP_BASE) / SC8551_BAT_OCP_LSB)
	       << SC8551_BAT_OCP_SHIFT;

	ret = regmap_update_bits(chip->regmap, SC8551_BAT_OCP_REG,
				 SC8551_BAT_OCP_MASK, data);
	if (ret)
...
static int ops_sc8551_get_charge_enable(struct charger_device *chg_dev,
					bool *enabled)
{
	struct sc8551 *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int ops_sc8551_is_bypass_enabled(struct charger_device *chg_dev,
					bool *enabled)
{
	struct sc8551 *chip = charger_get_data(chg_dev);
	int ret = 0;
...
static int ops_sc8551_get_bypass_support(struct charger_device *chg_dev,
					 bool *enabled)
{
	*enabled = 1;
	chr_err("%s %d\n", __func__, *enabled);
...
static int sc8551_register_charger(struct sc8551 *chip, int work_mode)
{
	...
	case SC8551_STANDALONE:
		chip->chg_dev =
			charger_device_register("cp_standalone", chip->dev,
						chip, &sc8551_chg_ops,
						&sc8551_standalone_chg_props);
		break;
	case SC8551_SLAVE:
		chip->chg_dev =
			charger_device_register("cp_slave", chip->dev, chip,
						&sc8551_chg_ops,
						&sc8551_slave_chg_props);
		break;
	case SC8551_MASTER:
		chip->chg_dev =
			charger_device_register("cp_master", chip->dev, chip,
						&sc8551_chg_ops,
						&sc8551_master_chg_props);
		break;
	default:
...
static int cp_vbus_get(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		       int *val)
{
	int ret = 0;
	u32 data = 0;
...
static int cp_ibus_get(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		       int *val)
{
	int ret = 0;
	u32 data = 0;
...
static int cp_tdie_get(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		       int *val)
{
	int ret = 0;
	u32 data = 0;
...
static int chip_ok_get(struct sc8551 *gm, struct mtk_cp_sysfs_field_info *attr,
		       int *val)
{
	if (gm)
		*val = gm->chip_ok;
...
static int sc8551_power_supply_init(struct sc8551 *chip, struct device *dev,
				    int work_mode)
{
...
static ssize_t sc8551_show_register(struct device *dev,
...
static ssize_t sc8551_show_adc(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct sc8551 *chip = dev_get_drvdata(dev);
	u8 tmpbuf[150];
...
static ssize_t sc8551_store_registers(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct sc8551 *chip = dev_get_drvdata(dev);
...
static ssize_t sc8551_show_log_level(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct sc8551 *chip = dev_get_drvdata(dev);
	int ret = 0;
...
static ssize_t sc8551_store_log_level(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct sc8551 *chip = dev_get_drvdata(dev);
...
static DEVICE_ATTR(register, S_IRUGO | S_IWUSR, sc8551_show_register,
		   sc8551_store_registers);
static DEVICE_ATTR(adc, S_IRUGO, sc8551_show_adc, NULL);
static DEVICE_ATTR(log_level, S_IRUGO | S_IWUSR, sc8551_show_log_level,
		   sc8551_store_log_level);
...
static int sc8551_init_irq(struct sc8551 *chip)
{
	...
	ret = request_irq(chip->irq, sc8551_interrupt,
...
static int sc8551_parse_dt(struct sc8551 *chip)
{
	...
	sc_info("%s parse config, [ac_ovp bus_ovp bus_ocp bat_ovp bat_ocp] = [%d %d %d %d %d]\n",
		chip->log_tag, chip->ac_ovp, chip->bus_ovp, chip->bus_ocp,
		chip->bat_ovp, chip->bat_ocp);
...
static int sc8551_init_adc(struct sc8551 *chip)
{
	...
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_CTRL_REG,
				 SC8551_DISABLE_IBUS_ADC_BIT, 0);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_VBUS_ADC_BIT, 0);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_VAC_ADC_BIT,
				 SC8551_DISABLE_VAC_ADC_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_VOUT_ADC_BIT,
				 SC8551_DISABLE_VOUT_ADC_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_VBAT_ADC_BIT, 0);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_IBAT_ADC_BIT,
				 SC8551_DISABLE_IBAT_ADC_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_ADC_FN_DISABLE_REG,
				 SC8551_DISABLE_TDIE_ADC_BIT, 0);
...
static int sc8551_init_device(struct sc8551 *chip)
{
	...
	ret = regmap_update_bits(chip->regmap, SC8551_BAT_OVP_REG,
				 SC8551_BAT_OVP_DIS_BIT,
				 SC8551_BAT_OVP_DIS_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_BAT_OCP_REG,
				 SC8551_BAT_OCP_DIS_BIT,
				 SC8551_BAT_OCP_DIS_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_BUS_OCP_UCP_REG,
				 SC8551_BUS_OCP_DIS_BIT,
				 SC8551_BUS_OCP_DIS_BIT);
	ret = regmap_update_bits(chip->regmap, SC8551_CTRL_REG,
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ERR_VALUE((unsigned long)ret)) {
		dev_err(&client->dev,
			"fail to detect sc8551 on i2c_bus(addr=0x%x), retry\n",
			client->addr);
		msleep(250);
		ret = i2c_smbus_read_byte_data(client, SC8551_PART_INFO_REG);
		if (IS_ERR_VALUE((unsigned long)ret)) {
			...
			if (IS_ERR_VALUE((unsigned long)ret)) {
				dev_err(&client->dev,
					"fail to detect sc8551 on i2c_bus(addr=0x%x)\n",
					client->addr);
				return -ENODEV;
			}
		}
