...
module_param_named(pdm_vbus_high_gap_bypass, pdm_vbus_high_gap_bypass, int,
		   0600);
...
module_param_named(switch2_1_bypass_enter_fcc_set,
		   switch2_1_bypass_enter_fcc_set, int, 0600);

static int switch2_1_bypass_exit_fcc_set = 6000;
module_param_named(switch2_1_bypass_exit_fcc_set, switch2_1_bypass_exit_fcc_set,
		   int, 0600);

static int switch1_1_single_enter_fcc_set = 1200;
module_param_named(switch1_1_single_enter_fcc_set,
		   switch1_1_single_enter_fcc_set, int, 0600);

static int switch1_1_single_exit_fcc_set = 3000;
module_param_named(switch1_1_single_exit_fcc_set, switch1_1_single_exit_fcc_set,
		   int, 0600);
...
static bool pdm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	for (i = 0; i < pdpm->cap.nr; i++) {
		if (pdpm->cap.type[i] == MTK_PD_APDO &&
		    (pdpm->cap.min_mv[i] >= 5000 || pdpm->cap.ma[i] <= 1750 ||
		     (pdpm->cap.maxwatt[i] / 1000000) == 33 ||
		     (pdpm->cap.maxwatt[i] / 1000000) == 35)) {
			pdm_info(
				"dump PDO min_mv=%d, max_mv=%d, ibus=%d, type=%d\n",
				pdpm->cap.min_mv[i], pdpm->cap.max_mv[i],
				pdpm->cap.ma[i], pdpm->cap.type[i]);
		} else if (pdpm->cap.type[i] == MTK_PD_APDO &&
			   pdpm->pd_verifed) {
			pdpm->pd_nobypass = 0;
			pdm_info(
				"dump PDO min_mv=%d, max_mv=%d, ibus=%d, type=%d\n",
		...
		if (pdpm->cap.max_mv[i] > MAX_VBUS_120W &&
		    pdpm->cap.ma[i] > MIN_IBUS_120W &&
		    (pdpm->cap.maxwatt[i] / 1000000) != 105) {
			pdpm->cp_4_1_mode = true;
		}

		if (pdpm->cap.max_mv[i] > MAX_VBUS_120W &&
		    pdpm->cap.ma[i] > MIN_IBUS_120W &&
		    !(pdpm->switch1_1_enable || pdpm->switch1_1_single_enable ||
		      pdpm->switch2_1_enable)) {
			...
			pdpm->apdo_min_vbus = pdpm->cap.min_mv[i];
			   ...
			   pdpm->cap.ma[i] >= pdpm->apdo_max_ibus) {
			pdpm->apdo_max_vbus = pdpm->cap.max_mv[i];
			pdpm->apdo_max_ibus = pdpm->cap.ma[i];
			pdpm->apdo_max_watt = pdpm->cap.maxwatt[i];
			pdpm->apdo_min_vbus = pdpm->cap.min_mv[i];
			if (pdpm->switch1_1_enable ||
			    pdpm->switch1_1_single_enable ||
			    pdpm->switch2_1_enable)
				pdpm->div_rate = 1;
			else
				pdpm->div_rate = 2;
	...
	pdm_info("pd_type = %d pd_nobypass=%d cp_4_1_mode=%d\n", pdpm->pd_type,
		 pdpm->pd_nobypass, pdpm->cp_4_1_mode);
	if (legal_pdo) {
...
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	pdm_info(
		"mt6375 vbus= %d, master_vbus=%d, master_ibus=%d, master_vbatt=%d, slave_ibus=%d, total_ibus=%d, master_enable=%d, slave_enable=%d, master_cp_mode=%d, slave_cp_mode=%d cycle_count=%d\n",
		...
		pdpm->master_cp_mode, pdpm->slave_cp_mode, pdpm->cycle_count);
	pdm_info(
		"soc=%d, vbatt=%d, ibatt=%d, jeita_index=%d, vbatt_step=%d, ibatt_step=%d, vbus_step=%d, ibus_step=%d, final_step=%d, target_fcc=%d, sw_cv=%d, charge_full=%d, input_suspend=%d, typec_burn=%d, i2c_error_count=%d\n",
		pdpm->soc, pdpm->vbat, pdpm->ibat, pdpm->jeita_chg_index,
		pdpm->vbat_step, pdpm->ibat_step, pdpm->vbus_step,
		pdpm->ibus_step, pdpm->final_step, pdpm->target_fcc,
		pdpm->sw_cv, pdpm->charge_full, pdpm->input_suspend,
		pdpm->typec_burn, pdpm->bms_i2c_error_count);
...
static void pdm_multi_mode_switch(struct usbpd_pm *pdm)
{
	if (pdm->soc <= pdm->dts_config.high_soc) {
		if (pdm->switch2_1_enable) {
			if (pdm->target_fcc >= switch2_1_exit_fcc_set &&
			    pdm->cp_4_1_mode) {
				pdm->switch_mode = true;
				pdm->switch2_1_enable = false;
			} else if (pdm->target_fcc <=
					   switch1_1_single_enter_fcc_set &&
				   !pdm->pd_nobypass && pdm->soc > 10) {
				pdm->switch_mode = true;
				pdm->switch2_1_enable = false;
				pdm->switch1_1_single_enable = true;
			} else if (pdm->target_fcc <=
					   switch2_1_bypass_enter_fcc_set &&
				   !pdm->pd_nobypass && pdm->soc > 10) {
				pdm->switch_mode = true;
				pdm->switch2_1_enable = false;
			...
			if (pdm->target_fcc <= switch1_1_single_enter_fcc_set &&
			    !pdm->pd_nobypass && pdm->soc > 10) {
				pdm->switch_mode = true;
				pdm->switch1_1_single_enable = true;
			} else if (pdm->target_fcc <=
	...
	if (pdm->master_chip_id == SC8561 && pdm->slave_chip_id == SC8561) {
		pdm->vbus_low_gap = (pdm->switch1_1_enable ||
				     pdm->switch1_1_single_enable) ?
					    pdm_vbus_low_gap_bypass :
				    pdm->switch2_1_enable ?
					    pdm_vbus_low_gap_21 :
					    pdm_vbus_low_gap_41_sc;
		pdm->vbus_high_gap = (pdm->switch1_1_enable ||
				      pdm->switch1_1_single_enable) ?
					     pdm_vbus_high_gap_bypass :
				     pdm->switch2_1_enable ?
					     pdm_vbus_high_gap_21 :
					     pdm_vbus_high_gap_41_sc;
	} else {
		pdm->vbus_low_gap = (pdm->switch1_1_enable ||
				     pdm->switch1_1_single_enable) ?
					    pdm_vbus_low_gap_bypass :
		...
		pdm->vbus_high_gap = (pdm->switch1_1_enable ||
				     ...
				     pdm->switch2_1_enable ?
					     pdm_vbus_high_gap_21 :
					     pdm_vbus_high_gap_41;
	}
	pdm->ibus_gap =
		(pdm->switch1_1_enable || pdm->switch1_1_single_enable) ?
			pdm_ibus_gap_bypass :
		pdm->switch2_1_enable ? pdm_ibus_gap_21 :
					pdm_ibus_gap_41;
	if (pdm->sconfig == 18 || pdm->sconfig == 17 || pdm->sconfig == 20 ||
	    pdm->sconfig == 9 || pdm->sconfig == 23 || pdm->sconfig == 4 ||
	    pdm->sconfig ==
		    34) //8K\4K\mgame\tgame\yuanshen\videochat\per-videochat
	{
	...
	pdm->entry_vbus =
		min(min(((pdm->vbat * pdm->div_rate) + pdm->vbus_low_gap),
			pdm->dts_config.max_vbus),
		    pdm->apdo_max_vbus);
	pdm->entry_ibus =
		min(min(((pdm->target_fcc / pdm->div_rate) + pdm->ibus_gap),
			pdm->dts_config.max_ibus),
		    pdm->apdo_max_ibus);
	if (pdm->soc > 85)
		pdm->entry_vbus = min(min(pdm->entry_vbus - 360,
					  pdm->dts_config.max_vbus),
				      pdm->apdo_max_vbus);
	pdm_err("%s [%d , %d , %d , %d , %d, %d] [%d, %d, %d , %d]\n", __func__,
		pdm->switch2_1_enable, pdm->switch1_1_enable,
		pdm->switch2_1_single_enable, pdm->switch1_1_single_enable,
		pdm->switch_mode, pdm->vbus_low_gap, pdm->vbus_high_gap,
		pdm->ibus_gap, pdm->entry_vbus, pdm->entry_ibus);
}

static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	int fv = 0, ibus_limit = 0, vbus_limit = 0, request_voltage = 0,
	    request_current = 0, final_step = 0, ret = 0, fv_vote = 0;
	...
	if (pdpm->sw_cv >= 4000) {
		...
		fv = fv < fv_vote ?
			     fv :
			     (pdpm->cycle_count > 100 ? fv_vote - 5 : fv_vote);
	}
	...
	pdpm->ibat_step = pdpm->vbat_step = pdpm->ibus_step = pdpm->vbus_step =
		0;
	ibus_limit =
		min(min((pdpm->target_fcc / pdpm->div_rate + pdpm->ibus_gap),
			apdo_max_ibus_temp),
	...
	if (pdpm->ibat < 0 && pdpm->total_ibus > 0 && pd_tune_count < 8 &&
	    apdo_max_ibus_temp >= (pdpm->target_fcc / pdpm->div_rate)) {
		fcc_ibatt_diff = ((-pdpm->ibat) > pdpm->target_fcc) ?
					 ((-pdpm->ibat) - pdpm->target_fcc) :
					 (pdpm->target_fcc - (-pdpm->ibat));
		if (fcc_ibatt_diff > 4000) {
			pdpm->final_step =
				pdpm->final_step * QUICK_RAISE_VOLT_STEP_DELTA;
			pd_tune_count++;
		}
...
static bool pdm_handle_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			...
			if (pdpm->switch_mode) {
				...
				pdm_info(
					"PD_PM_STATE_ENTRY, switch_mode trig src_caps\n");
			}
			break;
		} else {
			...
			msleep(200);
			charger_dev_is_powerpath_enabled(pdpm->charger_dev,
							 &power_path_en);
			if (power_path_en) {
				charger_dev_enable(pdpm->charger_dev, false);
			...
			if (pdpm->target_fcc > switch2_1_enter_fcc_set &&
			    pdpm->cp_4_1_mode && !pdpm->switch2_1_enable) {
				pdpm->cp_work_mode =
				...
				charger_dev_cp_device_init(pdpm->slave_dev,
							   CP_FORWARD_4_TO_1);
				charger_dev_cp_enable_adc(pdpm->master_dev,
							  true);
				charger_dev_cp_enable_adc(pdpm->slave_dev,
							  true);
				pdpm->div_rate = 4;
			} else if (pdpm->switch2_1_enable) {
				pdpm->cp_work_mode =
	...
	case PD_PM_STATE_INIT_VBUS:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			pdm_info(
				"[XMC_PDM] PDM_STATE_INIT_VBUS taper charge done\n");
		...
		if (pdpm->master_cp_vbus <=
		    pdpm->vbat * pdpm->div_rate + pdpm->vbus_low_gap) {
			pdpm->request_voltage += pdpm->div_rate * STEP_MV;
		} else if (pdpm->master_cp_vbus >=
			   pdpm->vbat * pdpm->div_rate + pdpm->vbus_high_gap) {
			pdpm->request_voltage -= pdpm->div_rate * STEP_MV;
		} else {
		...
		if (pdpm->switch1_1_enable || pdpm->switch1_1_single_enable)
			apdo_max_ibus_temp = pdpm->apdo_max_ibus * 80 / 100;
		pdpm->request_current =
			min(min(pdpm->target_fcc / pdpm->div_rate,
				apdo_max_ibus_temp),
			    pdpm->dts_config.max_ibus);
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
	...
	case PD_PM_STATE_ENABLE_CP:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			pdm_info(
				"[XMC_PDM] PDM_STATE_ENABLE_CP taper charge done\n");
			pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
			break;
		} else if (pdpm->sm_status == PDM_SM_HOLD) {
			pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
		...
		if (!pdpm->slave_cp_enable && !pdpm->switch1_1_single_enable &&
		    pdpm->pdm_active)
		...
		if (pdpm->slave_cp_mode != pdpm->cp_work_mode &&
		    !pdpm->switch1_1_single_enable)
			charger_dev_cp_set_mode(pdpm->slave_dev,
						pdpm->cp_work_mode);

		if (pdpm->master_cp_enable &&
		    (pdpm->slave_cp_enable ^ pdpm->switch1_1_single_enable) &&
		    pdpm->master_cp_mode == pdpm->cp_work_mode &&
		    (pdpm->slave_cp_mode == pdpm->cp_work_mode ||
		     pdpm->switch1_1_single_enable)) {
	...
	case PD_PM_STATE_EXIT:
		...
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
			adapter_dev_set_cap_xm(
				pinfo->pd_adapter, MTK_PD_APDO,
				max(DEFAULT_PDO_VBUS_1S, pdpm->apdo_min_vbus),
				min(DEFAULT_PDO_IBUS_1S, pdpm->apdo_max_ibus));
		msleep(500);
		charger_dev_get_vbus(pdpm->charger_dev, &pdpm->mt6375_vbus);
...
#endif
	pdm_info(
		"parse config, FV = %d, FV_FFC = %d, FCC = [%d %d %d], MAX_VBUS = %d, MAX_IBUS = %d, CV = [%d %d %d], ENTRY = [%d %d %d %d %d], PDO_GAP = [%d %d %d %d %d]\n",
		...
		pdpm->dts_config.low_tbat, pdpm->dts_config.high_tbat,
		pdpm->dts_config.high_vbat, pdpm->dts_config.high_soc,
		pdpm->dts_config.low_fcc, pdpm->vbus_low_gap,
		pdpm->vbus_high_gap, pdpm->ibus_gap,
