...
static void pdm_multi_mode_switch(struct usbpd_pm *pdm)
{
	...
	if (pdm->batt_drvdata->hifi_connected) {
		if (pdm->switch1_1_single_enable || pdm->switch1_1_enable) {
			pdm->switch_mode = true;
			pdm->cp_force_2_1_enable =
				true; //force cp mode to 2-1 mode if it's about to be 1-1 mode
			pdm->switch1_1_enable = false;
			pdm->switch1_1_single_enable = false;
			pdm->switch2_1_enable = true;
		}
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->pd_soft_reset) {
		...
		return PDM_SM_HOLD;
	} else if (pdpm->smart_chg[SMART_CHG_NAVIGATION].active_status &&
		   pdpm->soc >=
			   pdpm->smart_chg[SMART_CHG_NAVIGATION].func_val) {
		return PDM_SM_HOLD;
...
static bool pdm_handle_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			...
			if ((pinfo->product_name == COROT &&
			     pdpm->switch_mode) ||
			    (pinfo->product_name == ZIRCON &&
			     pdpm->switch_mode && !pdpm->hold_pdm_use_pmic)) {
	...
	case PD_PM_STATE_EXIT:
		...
		if (pdpm->charge_full) {
			msleep(1000);
			charger_dev_enable(pdpm->charger_dev, false);
		} else if (pinfo->night_charge_enable ||
			   pdpm->smart_chg[SMART_CHG_NAVIGATION].active_status)
			charger_dev_enable(pdpm->charger_dev, false);
	...
	if (!pdm_handle_sm(pdpm) && pdpm->pdm_active) {
		if (pdpm->no_delay) {
			...
			case PD_PM_STATE_TUNE:
				...
				else if (pdpm->pdm_slow_tuning &&
					 pinfo->product_name == ZIRCON)
					internal =
						PDM_SM_DELAY_1000MS; //FG ibat register is updated every 1000MS
				else
